...
- 긴 토픽도 전체 글을 수집한다. `/t/{id}.json` 첫 응답(보통 첫 20개) 이후의 글은
  `post_stream.stream`의 id를 `/t/{id}/posts.json`으로 40개씩 보충 조회해 저장한다.
- 드물게 응답에 없는 글(삭제 등)로 `len(posts) < posts_count`가 되면
  `토픽 N 부분 저장 (x/M)` 경고를 남긴다.

## 범위 밖 (이후 단계)

- LLM 요약/분류 리포트
- 검색/질의 에이전트, 임베딩/RAG, FTS5 전문검색
